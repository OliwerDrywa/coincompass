...
export function buildQuickSelectOptions(pinnedCurrencies, recentPairs) {
  return [
    ...pinnedCurrencies.map((code) => ({ value: `currency:${code}`, label: `★ ${code}` })),
    ...recentPairs.map(({ source, target }) => ({ value: `pair:${source}:${target}`, label: `↺ ${source} → ${target}` })),
  ]
}
